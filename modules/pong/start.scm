(begin
      ...
      (letrec (
	    ...
	    (chk_bx (lambda (p bl x)
			  ...
			  (begin
			    ...
			    (let ((speed (+ (abs (bx p)) (yeGetIntAt p "ball-acc")))
				  (max-s (yeGetIntAt p "max-speed"))
				  (dir (if (< (bx p) 0) 1 -1)))
			      (yeSetIntAt p "b_x_dir" (* dir (min speed max-s)))) )
			  ) ) )
	    (do_mv (lambda (p pl x y) (ywCanvasMoveObjXY pl x y) ))
	    (goal (lambda (p bl who)
		    (begin
		      (let ((base (yeGetIntAt p "base-speed")))
			(yeSetIntAt p "b_x_dir" (if (= who 0) base (- base)))
			(yeSetIntAt p "b_y_dir" (- (random 5) 2)))
  ...
  (define init_pong
	...
	(begin
	  ...
	  (yeCreateInt 0 p "up")
	  (yeCreateInt -3 p "b_x_dir")
	  (yeCreateInt 0 p "b_y_dir")
	  (yeCreateInt 0 p "p0-score")
	  (yeCreateInt 0 p "p1-score")
	  (yeCreateInt 2 p "win-score")
	  (yeCreateInt 1 p "ball-acc")
	  (yeCreateInt 8 p "max-speed")
	  (yeCreateInt 3 p "base-speed")
