...
int set_mem_atari(uint16_t addr, char val)
{
	...
	if (addr <= 0x2c || (addr >= SWCHA && addr <= T1024T)) {
		...
		case AUDC0:
		case AUDC1:
			tia.audc[addr - AUDC0] = val & 0x0f;
			break;
		case AUDF0:
		case AUDF1:
			tia.audf[addr - AUDF0] = val & 0x1f;
			break;
		case AUDV0:
		case AUDV1:
			tia.audv[addr - AUDV0] = val & 0x0f;
			break;
		case COLUP0:
			tia.col_p[0] = val;
			return 0;
...
static int process_inst(void)
{
	...
	if (current_emu_mode == ATARI_MODE) {
		...
		while (elapse > CYCLE_PER_SCANE_LINE) {
			atari_do_scan_line();
			/* 2 clocks audio per scanline: 3.58Mhz / 114 = 31400hz */
			tia_audio_gen(2, tia.audc, tia.audf, tia.audv);
...
void *fy_action(int nbArgs, void **args)
{
	...
	if (yevIsKeyDown(events, 'i') || yevIsKeyDown(events, 'o'))
		tia_audio_clear();
