...
Entity *ybytecode_exec(Entity *stack, int64_t *script)
{
  ...
  if (unlikely((!*script))) { // compille time :p
	...
	inst_compille(YB_CHECK_B_AND, check_b_and_comp, 3);
	...
	inst_compille(YB_SET_STRING, set_string, 2);
...
add:
 ...
 check_b_and_comp:
  if (yeIntCheckBAnd(yeGetByIdxDirect(stack, script[1]),
		     yeGetIntAt(stack, script[2]))) {
	  script = origin + script[3];
	  goto *((void *)*script);
  }
  script += 4;
